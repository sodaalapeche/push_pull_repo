...
def compute_records(A, B, exp_list):
    ...
    for res, L_mm, sand, alphaT, style, time_red, Sigma, Ta in exp_list:
        ...
        if sand =='fine' and L_mm == 3:
            tau = abs(beta) * (R2) ** 2 / (l * beta0 ** 2 * alphaT)
...
for res in results:
    ...
    if info is None:
        continue
    L_mm, sand = info
    # if L_mm == 3 and sand=="fine" :#or L_mm == 3:
    #     continue
...
# un seul point (τ moyen) avec barre d'erreur à 1σ par série en abscisse.
groups = {}
for rec in best_records:
...
for key, recs_g in groups.items():
    ...
    ax2.errorbar(Pe_g, tau_mean, yerr=1.96*tau_std,
...
tau_fit_p = 10**intercept * Pe_range**(slope + 2 * se_slope)
tau_fit_m = 10**intercept * Pe_range**(slope - 2 * se_slope)
...
ax2.set_ylabel(r"$\tau_D \cdot R^2 \;/ \tau_2$")

ax2.set_ylim(bottom=0.2,top=300)
