...
# ROI variance
R_EVAL_FACTOR = 1.6 # <-- demandé : 1.2 * rayon max RMS
...
def load_simulation_sigma_curve(vtk_dir=SIM_VTK_DIR):
    vtk_dir = Path(vtk_dir)
    case_dir = vtk_dir.parent
    csv_path = case_dir / "sigma_decay.csv"

    try:
        import openFoampost
        if hasattr(openFoampost, "load_sigma_decay"):
            df = openFoampost.load_sigma_decay(str(vtk_dir))
        elif hasattr(openFoampost, "compute_sigma_curve"):
            df = openFoampost.compute_sigma_curve(str(vtk_dir))
        else:
            df = None
    except Exception:
        df = None
    ...
    if df is None:
        if not csv_path.exists():
            raise FileNotFoundError(f"Missing simulation file: {csv_path}")
        df = pd.read_csv(csv_path)

    time_s = np.asarray(df["time"], dtype=float)

    if "sigma_over_sigma0" in df.columns:
        ...
        sigma0 = sigma[valid][0]
        sigma_norm = sigma / sigma0
    else:
        raise ValueError("Need column 'sigma_over_sigma0' or 'sigma' in simulation data")

    valid = np.isfinite(time_s) & np.isfinite(sigma_norm) & (sigma_norm > 0)
    time_s = time_s[valid]
    sigma_norm = sigma_norm[valid]

    return {"time_s": time_s, "sigma_norm": sigma_norm}
...
def build_master_sim_curve(sim_data, Ta_ref):
    ...
    # same time-origin convention as experiments: start at variance maximum
    i0 = int(np.nanargmax(sigma_norm))
    tau_sim = (time_s - time_s[i0]) / Ta_ref + 0.1

    valid = np.isfinite(tau_sim) & np.isfinite(sigma_norm) & (sigma_norm > 0)
...
def Ta_for_bead(timestamps, weights_g, D=0.055, L=0.01, f_billes=0.3,Tip="homo"):
    ...
    A_tot = np.pi * (D / 2)**2
    if Tip=="homo coarse":
        A_pore = A_tot * eps_sable*0.83
    elif Tip=="homo fine":
        A_pore = A_tot * eps_sable*0.7
    elif Tip=="0.1":
        A_pore = A_tot*eps_sable*(1-f_billes)*1.7
    elif Tip == "0.6":
        A_pore = A_tot*eps_sable*(1-f_billes)*0.86
        print("tick0.6)")
    elif Tip == "0.3 coarse":
        ...
        A_pore = A_tot*eps_sable*(1-f_billes)*1.1
    elif Tip == "0.1 coarse":
        A_pore = A_tot*eps_sable*(1-f_billes)*0.8
    elif Tip == "stokes 0.1":
        A_pore = A_tot*0.4
    elif Tip == "0.3 fine":
        A_pore = A_tot *eps_sable*(1-f_billes)*0.67
...
def roi_stats(frame, hex_mask, Xpix, Ypix, xc_pix, yc_pix, r_eval_pix):
    ...
    roi = (r2 <= (r_eval_pix ** 2))
    # print(r2*(0.06/512))
    # print("std: ",np.std(r2*(0.06/512)))
    # print("mean(r): ",np.mean(r2*(0.06/512)))
    vals = frame
    if vals.size == 0:
...
def process_experiment(root_folder, colonne="grande"):
    ...
    return {
        ...
        "yc_pix": yc_pix[i0],
        "I0": frame,
        "Sigma_max" :np.nanmax(var)  # <-- ADD THIS LINE
        # image utilisée pour le plot
     }
...
if __name__ == "__main__":
    ...
    np.save("/home/chorus/data_full4.npy", resultsA)

    print("Experiments OK :", len(results))
    sim_data = None
    SIM_TA = 6.8  # <-- put here the actual Ta [s] used to define endTime=32*Ta in this OpenFOAM run
    ...
    for res in results:
        ...
        # --- max sigma in the ROI (before normalization)
        Sigma = var / (mean ** 2)
        Sigma = Sigma / np.nanmax(Sigma)
        validS = np.isfinite(time) & np.isfinite(Sigma) & (Sigma > 0)
        if np.any(validS):
            axS.plot(time[validS], Sigma[validS], linestyle=None, **style)
        # --------- courbe Rayon mesuré
        # options d'affichage :
        #   - en mm : rad*1e3
        ...
        # axS.annotate(
        #     res["root"],
        #     xy=(time[validS][10], Sigma[validS][10]),
        #     xytext=(5, 0),
    ...
    if sim_data is not None and Ta_ref is not None:
        ...
        axS.plot(
            tau_sim, sigma_sim,
            None, color="k", lw=1.8, alpha=0.95,
            label=SIM_LABEL
        )
    ...
    # axS.set_xscale("log")
    axS.set_yscale("log")
    axS.set_xlim(left=0, right=32)
    axS.set_ylim(bottom=0.003, top=1.1)
    figS.tight_layout()
    ...
    # --- collect velocities
    u = np.array([0.01 / res["Ta"] for res in results], dtype=float)
    u_values = u / np.min(u)

    if len(u_values) > 1 and np.nanmax(u_values) > np.nanmin(u_values):
        norm = mpl.colors.Normalize(vmin=np.nanmin(u_values), vmax=np.nanmax(u_values))
    else:
        norm = mpl.colors.Normalize(vmin=u_values[0] * 0.9, vmax=u_values[0] * 1.1)

    # colormap monotone : noir -> rouge foncé -> rouge -> orange
    cmap = mpl.cm.hot
    # trier les expériences par u croissant
    order = np.argsort(u_values)

    for rank, idx in enumerate(order):
        res = results[idx]
        u_i = u_values[idx]
        color_i = cmap(norm(u_i))
        time = res["time"]
        mean = res["mean"]
        var = res["var"]
        Sigma = var / (mean ** 2)
        ...
        valid = np.isfinite(time0) & np.isfinite(Sigma) & (Sigma > 0)
    ...
    # optional simulation curve
    if sim_data is not None and Ta_ref is not None:
        tau_sim, sigma_sim = build_master_sim_curve(sim_data, Ta_ref)
        axU.plot(
            tau_sim, sigma_sim, color="k", lw=2.0, alpha=0.95,
            label=SIM_LABEL
        )
    ...
    sm.set_array([])
    cbar = figU.colorbar(sm, ax=axU)
    cbar.set_label(r"$u/u_{min}$", fontsize=14)

    # axes style
    axU.axvline(0, color="k", ls="--", alpha=0.5)
    axU.set_xlabel(r"$(t-t_0)/T_a$", fontsize=18)
    axU.set_ylabel(r"$\sigma_c^2/\mu_c^2$", fontsize=18)
    axU.set_yscale("log")
    axU.set_xlim(left=0.1, right=26)
    axU.set_ylim(bottom=0.01, top=1.1)
    axU.grid(True, which="both", ls="--", alpha=0.3)
