...
# Réplicat de 6 mm à exclure (mettre None pour tous les garder)
BAD_6MM_REPLICATE = 2
...
for L_mm in sorted(COLOR_MAP.keys()):
    legend_elements.append(
        Line2D([0], [0], marker="o", linestyle="None",
               color=COLOR_MAP[L_mm], mfc=COLOR_MAP[L_mm], mew=0.4, label=f"$d_2$ = {L_mm} mm — fine")
    )
    legend_elements.append(
        Line2D([0], [0], marker="D", linestyle="None",
               color=COLOR_MAP[L_mm], mfc="none",
                mew=1.0, alpha=0.65, label=f"$d_2$ = {L_mm} mm — coarse")
    )
...
legend2 = [
    Line2D([0], [0], marker="o", linestyle="None",
           color=COLOR_MAP[L_mm], mfc=COLOR_MAP[L_mm], mew=0.4, label=f"$d_2$ = {L_mm} mm")
