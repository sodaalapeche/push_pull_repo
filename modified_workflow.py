...
# ==========================================================
def roi_stats(frame, hex_mask, Xpix, Ypix, xc_pix, yc_pix, r_eval_pix, dx):
    # disque centré sur (xc_pix, yc_pix), restreint aux pixels réellement pris en compte
    ...
    area_m2 = float(np.count_nonzero(valid) * (dx ** 2))
    return float(np.mean(vals)), float(np.var(vals)), area_m2
...
def process_experiment(root_folder, colonne="grande"):
    ...
    area_m2 = np.full(n, np.nan, dtype=float)
    ...
    for i, f in enumerate(files):
        ...
        m, v, a = roi_stats(frame, hex_mask, Xpix, Ypix, xc, yc, r_eval_pix, dx)
        mean[i] = m
        var[i] = v
        area_m2[i] = a
        var_times_area[i] = v * a if np.isfinite(v) and np.isfinite(a) else np.nan
...
if __name__ == "__main__":
    ...
    for res in results:
        ...
        Sigma = res["var_times_area"] / (mean ** 2)
