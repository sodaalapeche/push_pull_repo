...
def process_experiment(root_folder, colonne="grande"):
    ...
    # --- centre fixe pour la ROI : centre du blob au temps où sigma_m est maximal
    i_center = int(np.nanargmax(sigma_m))
    xc0 = xc_pix[i_center] if np.isfinite(xc_pix[i_center]) else (W / 2.0)
    yc0 = yc_pix[i_center] if np.isfinite(yc_pix[i_center]) else (H / 2.0)

    # ROI fixe = disque centré en (xc0, yc0) intersecté avec le masque hexagonal
    r2 = (Xpix - xc0) ** 2 + (Ypix - yc0) ** 2
    disk_mask = r2 <= (r_eval_pix ** 2)
    roi_mask = disk_mask & hex_mask

    Npix_roi = int(np.sum(roi_mask))
    area_roi = Npix_roi * dx**2

    if Npix_roi == 0:
        raise RuntimeError("ROI vide après intersection disque ∩ hex_mask.")

    # --- 2) compute mean/var in the same fixed ROI for all frames
    mean = np.full(n, np.nan, dtype=float)
    var = np.full(n, np.nan, dtype=float)
    var_times_area = np.full(n, np.nan, dtype=float)
    ...
    for i, f in enumerate(files):
        ...
        m, v = roi_stats(frame, roi_mask)
        mean[i] = m
        var[i] = v
        var_times_area[i] = v * area_roi

    # --- choix de t0 comme max(var) (dans la ROI fixe)
    ...
    return {
        ...
        "mean": mean,
        "var": var,
        "var_times_area": var_times_area,
        "area_roi": area_roi,
        "n_pixels_roi": Npix_roi,
        "sigma_m": sigma_m,
        "Rmax_m": Rmax_m,
        "R_eval_m": R_eval_m,
        "mask": hex_mask,
...
if __name__ == "__main__":
    ...
    axS.set_ylabel(r"$A\,\sigma_c^2(t)/\mu_c^2$", fontsize=18)
    axS.grid(True, which="both", ls="--", alpha=0.3)
    # axS.set_xscale("log")
    axS.set_yscale("log")
    axS.set_xlim(left=0.1, right=40)
