...
A_MIN   = 10 # début minimum de la fenêtre
A_MAX   = 11  # début maximum
DUR_MIN =6 # durée minimale
DUR_MAX =7  # durée maximale (A + DUR_MAX <= A_MAX + DUR_MAX, mais limité par les données)
...
fit_label = rf"trendline : slope $= -0.15$ )"
...
# ax2.set_xscale("log")
# ax2.set_yscale("log")
ax2.set_xlabel(r"$Pe \propto d_2/d_1$")
ax2.set_ylabel(r"$\alpha_{\perp,M} / d_2$")
# / d_M^{0.85} \cdot d_m^{0.15}
ax2.minorticks_on()
ax2.grid(True, which='major', linestyle='--', alpha=0.7)
ax2.grid(True, which='minor', linestyle=':', alpha=0.4)
fig2.tight_layout()
plt.savefig('/home/chorus/dispersivity.pdf')
plt.show()