...
def process_experiment_rms(root_folder, colonne="grande"):
    ...
    for i, f in enumerate(files):
        ...
        var = np.nanvar(frame_var)
        mean = np.nanmean(frame_var)
        vars_I[i] = var/mean**2
...
# ==========================================================
def plot_rms_comparison(results,exclude_str="19"):

    style_map = {
        (10, "fine"): dict(marker="o", color="tab:blue"),
        (10, "coarse"): dict(marker="D", color="tab:blue", mfc="none"),
        (6, "fine"): dict(marker="o", color="tab:orange"),
        (6, "coarse"): dict(marker="D", color="tab:orange", mfc="none"),
        (3, "fine"): dict(marker="o", color="tab:red"),
        (3, "coarse"): dict(marker="D", color="tab:red", mfc="none"),
        (0, "fine"): dict(marker="o", color="tab:green"),
        (0, "coarse"): dict(marker="D", color="tab:green", mfc="none"),
    }
    ...
    for res in results:
        ...
        root_name = res.get("root", "").lower()

        if exclude_str is not None:
            if exclude_str.lower() in root_name:
                continue

        key = parse_label(res.get("label", ""))
        ...
        sigma = np.asarray(res["sigma_m"])
        var = np.asarray(res["var_I"])
        i0 = res["t0_index"]
        ...
        invR2 = 1.0 / (sigma_norm**2)
        ratio = var_norm * (sigma_norm**2)  # ← test physique

        valid = np.isfinite(time) & np.isfinite(var_norm) & np.isfinite(invR2)
        ...
        # ratio = ligne fine (optionnel mais très utile)
        # ax.plot(time[valid][::step],
        #         ratio[valid][::step],
        #         linestyle="-",
        #         color=style["color"],
    ...
    # --- AXES ---
    ax.set_xlabel(r"$(t - t_0)/t_a$", fontsize=14)
    ax.set_ylabel(r"$Var/Var_0$,  $(R_0/R)^2$", fontsize=14)

    ax.set_yscale("log")
    ax.set_xlim(0, 25)
    ax.set_ylim(0.003, 1.1)

    # ligne de référence physique
    ax.axhline(1, color='k', linestyle=':', alpha=0.7)
